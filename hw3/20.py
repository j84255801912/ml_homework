...
class hw(object):
    ...
    def compute_gradient(self, w, N, i):
        dim = len(myhw.data[0]['x'])
        w_all = [0]*dim
        eta_result = self.sigmoid((-1)*self.data[i]['y']*self.dot(w, self.data[i]['x']))
        for j in range(len(self.data[0]['x'])):
            w_all[j] += eta_result*self.data[i]['y']*self.data[i]['x'][j]
        w_all = [w_all[j] / float(N) for j in range(dim)]
        return w_all
...
#    print myhw.data
    ...
    w = [0]*dim
    for t in range(T):
        the_gradient = myhw.compute_gradient(w, N, t%N)
        w = [w[i] + eta*the_gradient[i] for i in range(dim)]
    # test
    testN = len(myhw.test)
    error = 0
    for i in range(testN):
        if myhw.sign(myhw.dot(w, myhw.test[i]['x'])) != myhw.test[i]['y']:
            error += 1
